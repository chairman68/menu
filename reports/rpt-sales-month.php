...
foreach ($Item as $value){
    ...
    if(!isset ($data[$value["name"]][(string)$value["date"]->format("dmY")])){
        //$data1=array($value["name"]=>array($value["date"]->format("dmY")=>0));
        $data+=$data;
        $data[$value["name"]]+=["itogo"=>0];
        $data[$value["name"]]+=["ParentID"=>0];
    }
}
...
foreach ($Item as $value){
    ...
    $data[$value["name"]][(string)$value["date"]->format("dmY")]+=$value["kolvo"];
